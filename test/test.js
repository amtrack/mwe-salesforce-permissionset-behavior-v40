...
describe('Retrieving a PermissionSet', function() {
  ...
  before(function() {
    assert.equal(child.spawnSync('force-dev-tool', ['deploy', '-d', 'test/unpackaged/setup'], {}).status, 0);
  });
  after(function() {
    assert.equal(child.spawnSync('force-dev-tool', ['deploy', '-d', 'test/unpackaged/teardown'], {}).status, 0);
  });
  describe('standalone', function() {
    it('should finish without errors', function() {
      assert.equal(child.spawnSync('force-dev-tool', ['retrieve', '-d', 'test/fixtures/permissionset-only'], {}).status, 0);
    });
  ...
  describe('with CustomFields', function() {
    it('should finish without errors', function() {
      assert.equal(child.spawnSync('force-dev-tool', ['retrieve', '-d', 'test/fixtures/permissionset-with-customfields'], {}).status, 0);
    });
    it('should only return entries associated with the PermissionSet', function() {
  ...
  describe('with a CustomObject', function() {
    it('should finish without errors', function() {
      assert.equal(child.spawnSync('force-dev-tool', ['retrieve', '-d', 'test/fixtures/permissionset-with-customobject'], {}).status, 0);
    });
    it('should only return entries associated with the PermissionSet', function() {
